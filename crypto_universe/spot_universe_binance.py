...
async def fetch_exchange_universe(timeout_seconds: float = 20.0) -> dict[str, Any]:
    ...
    for row in rows:
        normalized = normalize_binance_pair(row)
...
def normalize_binance_pair(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    permission_sets = normalize_permission_sets(row.get("permissionSets"))
    if permissions and "SPOT" not in permissions:
        return None
    if permission_sets and "SPOT" not in permission_sets:
        return None
    ...
    return {
        ...
        "flags": {
            ...
            "permissionSetsIncludeSpot": "SPOT" in permission_sets,
        },
    }
...
def build_parser():
    return build_output_parser(
        "Fetch Binance tradable spot universe as normalized JSON.",
